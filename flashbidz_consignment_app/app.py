...
def quick_checkout():
    """
    Simple 'Quick Checkout' screen:
    - Scan SKUs to add items to a cart (using your barcode scanner)
    - Edit prices if needed
    - Mark all as SOLD in one click
    """
    # Cart is stored as a list of item IDs in the session
    cart_ids = session.get("checkout_cart", [])
    ...
    if request.method == "POST":
        ...
        # --- SCAN / ADD ITEM BY SKU ---
        if action == "scan":
            sku = (request.form.get("sku") or "").strip()
            if not sku:
                flash("Scan a SKU or type one in.", "error")
                return redirect(url_for("quick_checkout"))

            item = Item.query.filter_by(sku=sku).first()
            if not item:
                flash(f"No item found with SKU {sku}.", "error")
                return redirect(url_for("quick_checkout"))

            if item.status == "sold":
                flash(f"Item {sku} is already marked as sold.", "error")
                return redirect(url_for("quick_checkout"))

            if item.id in cart_ids:
                flash(f"Item {sku} is already in the checkout list.", "warning")
                return redirect(url_for("quick_checkout"))

            cart_ids.append(item.id)
            session["checkout_cart"] = cart_ids
        ...
        # --- CLEAR CART ---
            ...
            session.modified = True
            flash("Checkout list cleared.")
            return redirect(url_for("quick_checkout"))

        # --- COMPLETE SALE FOR ALL ITEMS IN CART ---
        elif action == "complete":

            def dollars_to_cents(val: str):
                val = (val or "").strip()
                if not val:
                    return None
                try:
            ...
            sale_date = parse_date(sale_date_str) if sale_date_str else date.today()

            ids = request.form.getlist("item_id")
            if not ids:
                flash("No items to sell.", "error")
                return redirect(url_for("quick_checkout"))
            ...
            for sid in ids:
                ...
                price_str = request.form.get(f"price_{iid}", "").strip()
                ...
                item.status = "sold"
                item.sale_price_cents = price_cents
                item.sale_date = sale_date
                if buyer_name:
            ...
            session["checkout_cart"] = []
            session.modified = True
            flash(f"Recorded {sold_count} sale(s).")
            return redirect(url_for("items_list"))

    # ---------- GET (or after POST redirect): build cart rows ----------
    cart_items = []
    subtotal = 0.0
    ...
    if cart_ids:
        for iid in cart_ids:
            ...
            # Default price for checkout:
            # Asking price → list price → previous sale price → cost → 0
            cents_val = (
                item.asking_cents
                or item.price_cents
                or item.sale_price_cents
                or item.cost_cents
                or 0
            )
            dollars_val = cents_val / 100.0
            subtotal += dollars_val
